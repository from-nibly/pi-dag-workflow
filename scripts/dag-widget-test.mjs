...
const hydrationIntervals = new FakeIntervals();
let releaseHydration;
const hydrationRead = new Promise((resolve) => { releaseHydration = resolve; });
let hydrationReads = 0;
const hydrationController = new DagWidgetControllerV2({
  async read() { hydrationReads += 1; await hydrationRead; return { kind: "projection", projection, fresh: true, diagnostic: null }; },
  requestRender() {},
  scheduleInterval: hydrationIntervals.schedule,
  clearScheduledInterval: hydrationIntervals.clear,
});
const hydration = hydrationController.start();
await Promise.resolve();
assert.equal(hydrationReads, 1, "controller start begins initial hydration without waiting for a timer");
assert.equal(hydrationIntervals.count(1_000), 0, "periodic refresh stays disabled while initial hydration is pending");
releaseHydration();
await hydration;
assert.equal(hydrationIntervals.count(1_000), 1, "periodic refresh starts only after initial hydration settles");
...
registerCanonicalDagRuntime(pi, {
  ...
  async resumeBound() {},
  async status() { await initialStatus; return { projection, stale: null, state: { completion: { state: "open" }, current: { run: "active" } } }; },
...
assert.equal(typeof widgetFactory, "function", "interactive TUI session installs the V2 widget without awaiting its initial status read");
const component = widgetFactory({ terminal: { rows: 24 }, requestRender() {} });
assert.deepEqual(component.render(80), [], "pending initial hydration leaves the passive widget empty instead of blocking startup");
...
const firstStart = startHandler({}, firstContext).then(() => { firstStartSettled = true; });
await firstStart;
assert.equal(firstStartSettled, true, "a slow conductor wake does not block session startup");
assert.equal(firstSessionCalls.filter(({ value }) => typeof value === "function").length, 1, "the current session installs its widget before conductor hydration settles");
await startHandler({}, secondContext);
assert.equal(firstSessionCalls.at(-1).value, undefined, "a newer session disposes the prior widget while its conductor hydration is pending");
releaseOldResume();
await Promise.resolve();
assert.equal(secondSessionCalls.filter(({ value }) => typeof value === "function").length, 1, "the current session alone retains a widget after overlapping startup");
for (const handler of overlapPi.handlers.get("session_shutdown") ?? []) await handler();

console.log("DAG widget V2 tests OK: responsive graph layout, nonblocking hydration, static activity, deduplicated refresh, and disposal fencing pass");
