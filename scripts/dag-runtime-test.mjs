...
for (const attemptState of ["failed", "lost", "ambiguous", "quarantined"]) { const terminal = clone(schedulableRun); terminal.workItems["item-api"].current = "active"; terminal.workItems["item-api"].currentStage = "F0"; terminal.workItems["item-api"].stages.F0.state = "failed"; terminal.workItems["item-api"].stages.F0.currentAttemptId = `attempt-${attemptState}`; terminal.stageAttempts[`attempt-${attemptState}`] = { stageAttemptId: `attempt-${attemptState}`, state: attemptState }; rehashRun(terminal); const terminalDecision = scheduleDagRunV1(plan, terminal); const projected = projectDagExecutionV1(plan, terminal, terminalDecision); assert.equal(projected.nodes[0].glyph, "!", `terminal ${attemptState} attempts render attention rather than in-flight`); assert.equal(terminalDecision.selected.length, 0, `terminal ${attemptState} evidence cannot be readmitted without explicit retry authority`); }
