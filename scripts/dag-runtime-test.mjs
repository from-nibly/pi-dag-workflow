...
const activeWorkerProjection = { projectionHash: H("4"), workers: [{ storageId: activeBinding.workerStorageId, launchOwnerSessionId: activeBinding.launchOwnerSessionId, workerId: activeBinding.workerId, attemptNumber: activeBinding.attemptNumber, attemptNonce: activeBinding.attemptNonce, configHash: activeBinding.configHash, terminalStatus: null, processDisposition: activeBinding.processDisposition, retrySafe: activeBinding.retrySafe, resultHash: activeBinding.resultHash }] };
...
assert.equal(activeExecutionProjectionV2.nodes.find(({ workItemId }) => workItemId === "item-api").worker?.processDisposition, "live", "V2 exposes only the exact joined bound-worker process disposition");
...
const unboundWorkerProjection = { projectionHash: H("c"), workers: [{ storageId: "unbound", launchOwnerSessionId: "unbound-session", workerId: "generic-worker", attemptNumber: 1, attemptNonce: "0123456789abcdef", configHash: H("d"), terminalStatus: "succeeded", processDisposition: "dead", retrySafe: true, resultHash: H("e") }] };
...
try {
  ...
  assert.equal(afterRestart.state.revision, revision + 1); assert.equal(afterRestart.state.owner.ownerEpoch, cancelledLifecycle.state.owner.ownerEpoch + 1, "fresh same-process service fences the prior owner generation"); assert.equal(cancelledLifecycle.adapters.procedureCalls.get(cancelledAttemptId), 1, "restart observes the sealed cancelled attempt without a retry loop");
} finally {
