    ...
    adapters: {
      worker: {
        ...
        async readTerminalExact(binding, state) {
          ...
          return { completionId: `completion-${binding.workerId}`, terminalStatus: "succeeded", processDisposition: "dead", retrySafe: true, workerOutput: { outputRepositoryId: item.writeRepositoryId, outputCommonDirIdentityHash: canonicalHash({ repo, common: await git(repo, ["rev-parse", "--git-common-dir"]) }), outputWorktreeIdentityHash: canonicalHash({ workspace: launch.workspace }), outputSourceBase: sourceBase, outputCommit: output.commit, outputTree: output.tree, outputObjectFormat: output.commit.length === 40 ? "sha1" : "sha256", candidateObservedAt: AT } };
        },
