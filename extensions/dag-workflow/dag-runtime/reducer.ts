...
const WorkerBindingInputSchema = StrictObject({
  ...
  completionId: Type.Null(), resultHash: Type.Null(), processDisposition: Type.Literal("live"), retrySafe: Type.Literal(false),
});
...
function validateObservationFacts(state: DagRunStateV1, input: DagRunInputV1, context: DagRunValidationContextV1): string | null {
  ...
  if (input.type === "record_cancellation") {
    ...
    for (const workerResult of payload.workerResults) {
      ...
      if (!binding || !attempt || workerResult.result.kind !== "worker_result" || workerResult.result.bytes !== Buffer.byteLength(canonicalStringify(fact)) || fact?.kind !== "worker_result" || fact.hash !== workerResult.result.hash || fact.planHash !== state.identity.planHash || fact.runId !== state.runId || fact.runNonce !== state.runNonce || fact.workItemId !== attempt.workItemId || fact.stage !== attempt.stage || fact.stageAttemptId !== attempt.stageAttemptId || fact.launchIntentId !== binding.launchIntentId || fact.workerStorageId !== binding.workerStorageId || fact.launchOwnerSessionId !== binding.launchOwnerSessionId || fact.workerId !== binding.workerId || fact.attemptNumber !== binding.attemptNumber || fact.attemptNonce !== binding.attemptNonce || fact.configHash !== binding.configHash || !["succeeded", "needs_attention", "failed", "cancelled", "lost"].includes(fact.terminalStatus) || fact.processDisposition !== "dead" || !fact.retrySafe || !validWorkerGitOutputIdentity(fact, attempt, state, context)) return "cancellation worker result must prove exact attempt-bound terminal identity, Git output shape, and retry-safe process death";
    }
  ...
  switch (input.type) {
    ...
    case "put_effect_intent": {
      ...
      if (effect.kind === "cleanup_worktree") {
        ...
        if (!attempt?.terminalAt || !binding || binding.resultHash !== workerResultHash || result?.kind !== "worker_result" || result.processDisposition !== "dead" || result.retrySafe !== true || result.outputRepositoryId !== repositoryId || result.outputCommonDirIdentityHash === null || result.outputWorktreeIdentityHash === null || effect.subject.kind !== "work_item" || effect.subject.id !== attempt.workItemId || effect.procedureClass !== "idempotent" || effect.effectScopeId !== null || effect.requestHash !== requestHash) return precondition("cleanup_worktree requires exact terminal retry-safe worker/result/repository/worktree request identity");
      } else {
...
function sealStageAttempt(state: DagRunStateV1, input: DagRunInputV1, payload: any, context: DagRunValidationContextV1, notices: DagRunTransitionNoticeV1[], f8: boolean): { code: DagRunRejectCodeV1; message: string } | null {
  ...
  if (resultFact?.terminalStatus === "cancelled") {
    ...
    if (!binding || binding.resultHash !== resultFact.hash || binding.processDisposition !== "dead" || !binding.retrySafe || resultFact.processDisposition !== "dead" || !resultFact.retrySafe || derivedDisposition !== "BLOCKED") return precondition("cancelled worker seal requires exact bound retry-safe process death and canonical BLOCKED derivation");
  ...
  if (["F2", "F5", "F7"].includes(attempt.stage)) {
    ...
    if (attempt.stage === "F7" && observation.cleanliness !== "clean") return precondition("F7 requires fresh clean exact-tree evidence");
