...
export class DagReducerGitIntegrationDriverV1 implements DagIntegrationReconciliationAdapterV1 {
  ...
  async reconcileExact(input: Parameters<DagIntegrationReconciliationAdapterV1["reconcileExact"]>[0]): Promise<void> {
    const state = await this.readExact(input.state, input.reservation.reservationId);
    ...
    if (!attemptId) {
      await this.reserve(state, input.plan, reservation, input.repositoryRoot);
      return;
    }
    ...
    if (entry.state === "composing" && compositionEffect?.state === "intended") {
      await this.mutate(state, "mark_effect_dispatching", "command", { effectId: compositionEffect.effectId, expectedDispatchCount: compositionEffect.dispatchCount }, `dispatch-${compositionEffect.effectId}-${compositionEffect.dispatchCount}`);
      await this.hit("after_composition_dispatch", { integrationAttemptId: attemptId });
      return;
    }
    if (entry.state === "composing" && compositionEffect?.state === "dispatching") {
      await this.compose(state, input.plan, reservation, attemptId, input.repositoryRoot);
      return;
    }
    if (entry.state === "verifying_prefix") {
      await this.verify(state, input.plan, reservation, attemptId, input.repositoryRoot);
      return;
    }
    if (entry.state === "landing" && !attempt.landingEffectId) {
      await this.prepareLanding(state, reservation, attemptId);
      return;
    }
    if (entry.state === "landing" && attempt.landingEffectId) {
      const landingEffect = state.effects[attempt.landingEffectId];
      if (landingEffect?.state === "intended") {
        await this.mutate(state, "mark_effect_dispatching", "command", { effectId: landingEffect.effectId, expectedDispatchCount: landingEffect.dispatchCount }, `dispatch-${landingEffect.effectId}-${landingEffect.dispatchCount}`);
        await this.hit("after_landing_dispatch", { integrationAttemptId: attemptId });
        return;
      }
      if (landingEffect?.state === "reconciled" && landingEffect.reconciliation === "proven_absent") {
        await this.mutate(state, "retry_effect_dispatch", "command", { effectId: landingEffect.effectId, expectedDispatchCount: landingEffect.dispatchCount, reason: "uncertain_acknowledgement" }, `retry-${landingEffect.effectId}-${landingEffect.dispatchCount}`);
        return;
      }
      if (landingEffect?.state === "dispatching") {
        await this.land(state, input.plan, reservation, attemptId, input.repositoryRoot);
        return;
      }
    }
    if (attempt.landingState === "reconciled") {
      await this.accept(state, input.plan, reservation, attemptId, input.repositoryRoot);
      return;
    }
  ...
  private async reserve(state: DagRunStateV1, plan: CanonicalDagPlanV1, reservation: any, repositoryRoot: string): Promise<void> {
    const item = state.workItems[reservation.workItemId];
    const train = state.integrationTrains[reservation.repositoryId];
    ...
    const request = await this.request(state, plan, integrationAttemptId, item, train, repositoryRoot);
    const binding = await preflightBoundRepositoryV1(request);
    const observed = await observeTarget(repositoryRoot, state.repositories[reservation.repositoryId].targetRef);
    ...
    await this.publishFact(bindingFact);
    await this.hit("after_repository_fact", { integrationAttemptId, repositoryBindingFactHash: bindingFact.hash });
    ...
    const fresh = await this.assertAuthority(state, reservation.reservationId, repositoryRoot, train.expectedTarget);
    ...
    }, `reserve-${integrationAttemptId}`);
    await this.hit("after_integration_reserve", { integrationAttemptId });
  }

  private async compose(state: DagRunStateV1, plan: CanonicalDagPlanV1, reservation: any, attemptId: string, repositoryRoot: string): Promise<void> {
    const attempt = state.integrationAttempts[attemptId];
    const item = state.workItems[reservation.workItemId];
    const effect = state.effects[attempt.compositionEffectId];
    const request = await this.request(state, plan, attemptId, item, state.integrationTrains[reservation.repositoryId], repositoryRoot, attempt);
    const binding = await preflightBoundRepositoryV1(request);
    const lock = await this.acquireOperationLock(state, reservation.reservationId, request, binding, attempt.expectedTarget, `lock-compose-${attemptId}`);
    try {
      const refs = privateRefs(request);
      ...
      for (const [role, ref, oid] of anchors) {
        await this.assertAuthority(state, reservation.reservationId, repositoryRoot, attempt.expectedTarget);
        await ensurePrivateGitRefV1(binding, ref, oid, operationGuard(`${effect.effectId}-${role}`, state.owner.ownerEpoch, "anchor_ref", { commonDirIdentityHash: binding.commonDirIdentityHash, refHash: canonicalHash(ref), oid }));
      }
      await this.assertAuthority(state, reservation.reservationId, repositoryRoot, attempt.expectedTarget);
      ...
      try {
        ...
        proposal = await composeGitProposalV1(request, binding, { effectId: effect.effectId, requestHash: canonicalHash({ kind: "compose", payload: successorGuardPayload }), ownerEpoch: state.owner.ownerEpoch });
      } catch (error) {
        ...
        await this.publishFact(conflictFact);
        const fresh = await this.assertAuthority(state, reservation.reservationId, repositoryRoot, attempt.expectedTarget);
        await this.mutate(fresh, "record_git_composition_conflict", "observation", { integrationAttemptId: attemptId, compositionFactHash: conflictFact.hash, conflictClass: error.code === "COMPOSITION_CONFLICT" ? "mechanical" : "ambiguous" }, `composition-conflict-${attemptId}`);
        return;
      }
      await this.hit("after_composition_git", { integrationAttemptId: attemptId, composedCommit: proposal.composed.commit });
      for (const [role, ref] of [["composed", refs.composed], ["proposal", refs.proposal]] as const) {
        await this.assertAuthority(state, reservation.reservationId, repositoryRoot, attempt.expectedTarget);
        await ensurePrivateGitRefV1(binding, ref, proposal.composed.commit, operationGuard(`${effect.effectId}-${role}`, state.owner.ownerEpoch, "anchor_ref", { commonDirIdentityHash: binding.commonDirIdentityHash, refHash: canonicalHash(ref), oid: proposal.composed.commit }));
      }
      ...
      const privateFacts = await Promise.all(Object.entries(refs).map(async ([role, targetRef]) => {
        ...
        await this.publishFact(fact); return fact;
      }));
      ...
      await this.publishFact(compositionFact);
      await this.hit("after_composition_facts", { integrationAttemptId: attemptId, compositionFactHash: compositionFact.hash });
      const fresh = await this.assertAuthority(state, reservation.reservationId, repositoryRoot, attempt.expectedTarget);
      ...
      await this.hit("after_composition_commit", { integrationAttemptId: attemptId, compositionFactHash: compositionFact.hash });
    } finally {
      await lock.release();
    }
  }

  private async verify(state: DagRunStateV1, plan: CanonicalDagPlanV1, reservation: any, attemptId: string, repositoryRoot: string): Promise<void> {
    const prefixComplete = await this.reconcileValidationPhase(state, plan, reservation, attemptId, repositoryRoot, "prefix");
    if (!prefixComplete) return;
    const afterPrefix = await this.store.read(this.context);
    const finalComplete = await this.reconcileValidationPhase(afterPrefix, plan, reservation, attemptId, repositoryRoot, "final");
    ...
    const request = await this.request(freshState, plan, attemptId, freshState.workItems[reservation.workItemId], freshState.integrationTrains[reservation.repositoryId], repositoryRoot, attempt);
    const binding = await preflightBoundRepositoryV1(request);
    await this.assertAuthority(freshState, reservation.reservationId, repositoryRoot, attempt.expectedTarget);
    ...
    await this.hit("after_verification_facts", { integrationAttemptId: attemptId, proposalVerificationFactHash: fact.hash });
    const fresh = await this.assertAuthority(freshState, reservation.reservationId, repositoryRoot, attempt.expectedTarget);
    await this.mutate(fresh, "record_proposal_verification", "observation", { integrationAttemptId: attemptId, proposalVerificationFactHash: fact.hash, ...closure }, `verification-${attemptId}`);
  }

  private async reconcileValidationPhase(state: DagRunStateV1, plan: CanonicalDagPlanV1, reservation: any, attemptId: string, repositoryRoot: string, phase: "prefix" | "final"): Promise<boolean> {
    const attempt = state.integrationAttempts[attemptId];
    const trainPlan = plan.constraints.integrationTrains.find(({ repositoryId }) => repositoryId === reservation.repositoryId)!;
    ...
    if (!effect) {
      ...
      await this.mutate(state, "put_effect_intent", "command", { effect }, `validation-intent-${effectId}`);
      await this.hit(`after_${phase}_validation_intent` as DagGitIntegrationDriverFailpointV1, { integrationAttemptId: attemptId, phase, effectId, requestHash });
      return false;
    }
    ...
    if (effect.state === "intended") {
      await this.mutate(state, "mark_effect_dispatching", "command", { effectId, expectedDispatchCount: effect.dispatchCount }, `validation-dispatch-${effectId}-${effect.dispatchCount}`);
      await this.hit(`after_${phase}_validation_dispatch` as DagGitIntegrationDriverFailpointV1, { integrationAttemptId: attemptId, phase, effectId, requestHash });
      return false;
    }
    if (effect.state === "dispatching") {
      let execution = await this.findDurableValidationObservation(effect);
      if (!execution) {
        const request = await this.request(state, plan, attemptId, state.workItems[reservation.workItemId], state.integrationTrains[reservation.repositoryId], repositoryRoot, attempt);
        const binding = await preflightBoundRepositoryV1(request);
        await this.assertAuthority(state, reservation.reservationId, repositoryRoot, attempt.expectedTarget);
        execution = await executeValidationProfile(profile, { phase, profileId, profileHash }, state, trainPlan.trainId, reservation.repositoryId, attemptId, attempt.composedTree!, binding, request.controlRoot, repositoryRoot, () => state.updatedAt, effect);
        await this.publishFact(execution);
        await this.hit(`after_${phase}_validation_result` as DagGitIntegrationDriverFailpointV1, { integrationAttemptId: attemptId, phase, effectId, requestHash, executionObservationHash: execution.hash });
      }
      const fresh = await this.store.read(this.context);
      await this.mutate(fresh, "record_effect_execution", "observation", { effectId, executionObservationHash: execution.hash }, `validation-result-${effectId}-${execution.hash.slice(HASH_PREFIX.length, HASH_PREFIX.length + 12)}`);
      return false;
    }
    if (effect.state === "observed") {
      ...
      await this.publishFact(reconciliation);
      const fresh = await this.store.read(this.context);
      await this.mutate(fresh, "record_effect_observation", "observation", { effectId, observationHash: reconciliation.hash, reconciliation: "applied_exact", terminalState: "reconciled" }, `validation-reconcile-${effectId}`);
      await this.hit(`after_${phase}_validation_reconcile` as DagGitIntegrationDriverFailpointV1, { integrationAttemptId: attemptId, phase, effectId, requestHash, reconciliationHash: reconciliation.hash });
      return false;
    }
  ...
  private async findDurableValidationObservation(effect: any): Promise<any | null> {
  ...
  private async prepareLanding(state: DagRunStateV1, reservation: any, attemptId: string): Promise<void> {
    const attempt = state.integrationAttempts[attemptId];
    ...
    await this.mutate(state, "prepare_git_landing", "command", { integrationAttemptId: attemptId, landingEffect: effect, intendedLandedTree: attempt.composedTree }, `landing-intent-${attemptId}`);
    await this.hit("after_landing_intent", { integrationAttemptId: attemptId });
  }

  private async land(state: DagRunStateV1, plan: CanonicalDagPlanV1, reservation: any, attemptId: string, repositoryRoot: string): Promise<void> {
    const attempt = state.integrationAttempts[attemptId];
    const effect = state.effects[attempt.landingEffectId!];
    const request = await this.request(state, plan, attemptId, state.workItems[reservation.workItemId], state.integrationTrains[reservation.repositoryId], repositoryRoot, attempt);
    const binding = await preflightBoundRepositoryV1(request);
    const lock = await this.acquireOperationLock(state, reservation.reservationId, request, binding, null, `lock-land-${attemptId}`);
    try {
      await this.hit("before_landing", { integrationAttemptId: attemptId });
      const freshBefore = await this.assertAuthority(state, reservation.reservationId, repositoryRoot, null);
      const before = await observeTarget(repositoryRoot, state.repositories[reservation.repositoryId].targetRef);
      ...
      else reconciliation = await landOrReconcileBoundWorktreeV1(binding, state.repositories[reservation.repositoryId].targetRef, attempt.expectedTarget, attempt.intendedLandedTree!, { effectId: effect.effectId, requestHash: effect.requestHash, ownerEpoch: freshBefore.owner.ownerEpoch });
      const observed = await observeTarget(repositoryRoot, state.repositories[reservation.repositoryId].targetRef);
      ...
      await this.hit("after_landing_git", { integrationAttemptId: attemptId, reconciliation });
      ...
      await this.publishFact(fact);
      await this.hit("after_landing_fact", { integrationAttemptId: attemptId, landingObservationFactHash: fact.hash });
      const fresh = await this.assertAuthority(state, reservation.reservationId, repositoryRoot, null);
      await this.mutate(fresh, "record_git_landing_reconciliation", "observation", { integrationAttemptId: attemptId, landingObservationFactHash: fact.hash, reconciliation }, `landing-observation-${attemptId}-${fact.hash.slice(HASH_PREFIX.length, HASH_PREFIX.length + 12)}`);
      if (reconciliation === "proven_absent") await this.hit("after_landing_proven_absent_commit", { integrationAttemptId: attemptId, landingObservationFactHash: fact.hash });
    } finally {
      await lock.release();
    }
  }

  private async accept(state: DagRunStateV1, plan: CanonicalDagPlanV1, reservation: any, attemptId: string, repositoryRoot: string): Promise<void> {
    const attempt = state.integrationAttempts[attemptId];
    const item = state.workItems[reservation.workItemId];
    const bindingFact: any = await this.store.readImmutableFact(attempt.repositoryBindingFactHash);
    const observedBinding = await readRepositoryBindingIdentityV1(repositoryRoot);
    ...
    const privateFacts = await Promise.all(attempt.privateRefFactHashes.map((hash) => this.store.readImmutableFact(hash) as Promise<any>));
    ...
    await this.publishFact(transactionFact);
    ...
    await this.publishFact(integrationFact);
    await this.hit("after_receipt_facts", { integrationAttemptId: attemptId, transactionReceiptFactHash: transactionFact.hash, integrationReceiptHash: integrationFact.hash });
    const fresh = await this.assertAuthority(state, reservation.reservationId, repositoryRoot, attempt.intendedLandedTree!);
    await this.mutate(fresh, "accept_integration_receipt", "observation", { integrationAttemptId: attemptId, integrationReceiptHash: integrationFact.hash, transactionReceiptHash: receipt.receiptHash, transactionReceiptFactHash: transactionFact.hash }, `accept-${attemptId}`);
  }

  private async request(state: DagRunStateV1, plan: CanonicalDagPlanV1, attemptId: string, item: any, train: any, repositoryRoot: string, attempt?: any): Promise<GitIntegrationRequestV1> {
    const expectedRepositoryBinding = await readRepositoryBindingIdentityV1(repositoryRoot);
  ...
  private async acquireOperationLock(state: DagRunStateV1, reservationId: string, request: GitIntegrationRequestV1, binding: RepositoryBindingV1, expectedTarget: any, effectId: string) {
    await mkdir(request.controlRoot, { recursive: true }); await mkdir(request.artifactRoot, { recursive: true });
    await this.assertAuthority(state, reservationId, request.repositoryRoot, expectedTarget);
    ...
    return acquireGitIntegrationLockV1(request, binding, operationGuard(effectId, state.owner.ownerEpoch, "acquire_lock", payload));
  }

  private async readExact(delegated: DagRunStateV1, reservationId: string): Promise<DagRunStateV1> {
    const fresh = await this.store.read(this.context);
  ...
  private async assertAuthority(expected: DagRunStateV1, reservationId: string, repositoryRoot: string, expectedTarget: any | null): Promise<DagRunStateV1> {
    const fresh = await this.store.read(this.context);
    ...
    if (expectedTarget) {
      const target = await observeTarget(repositoryRoot, fresh.repositories[reservation.repositoryId].targetRef);
  ...
  private async publishFact(fact: any): Promise<void> {
  ...
  private async mutate(state: DagRunStateV1, type: string, kind: "command" | "observation", payload: any, slot: string): Promise<DagRunStateV1> {
  ...
  private async hit(point: DagGitIntegrationDriverFailpointV1, context: Record<string, unknown>): Promise<void> { await this.failpoint?.(point, context); }
}
...
async function executeValidationProfile(
  ...
  try {
    try {
      ...
      const existingCommit = await git(worktreeRoot, ["rev-parse", "HEAD"]);
      ...
      await git(repositoryRoot, ["worktree", "add", "--detach", worktreeRoot, tree.commit]);
      materialized = true;
    }
    const identity = await stat(worktreeRoot); const canonicalWorktree = await realpath(worktreeRoot);
    const before = await validationWorktreeIdentity(worktreeRoot);
    ...
    try {
      const result = await run(executable, profile.argv.slice(1), { cwd: worktreeRoot, env: { ...profile.environment }, timeout: profile.timeoutMs, maxBuffer: MAX_VALIDATION_OUTPUT, encoding: "utf8", windowsHide: true });
    ...
    const after = await validationWorktreeIdentity(worktreeRoot);
...
async function validationWorktreeIdentity(cwd: string): Promise<{ status: string; commit: string; tree: string }> {
  const status = await git(cwd, ["status", "--porcelain=v2", "--untracked-files=all"]);
  const commit = await git(cwd, ["rev-parse", "HEAD"]); const tree = await git(cwd, ["rev-parse", "HEAD^{tree}"]);
