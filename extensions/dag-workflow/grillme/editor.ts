...
export class GrillMeEditor extends CustomEditor {
  constructor(tui: TUI, theme: EditorTheme, keybindings: KeybindingsManager, private ctx: ExtensionContext) {
  ...
  render(width: number): string[] {
    ...
    add(rule);
    add(theme.fg("accent", ` GrillMe ${session.fileNumber} `) + theme.fg("muted", `Q ${session.currentIndex + 1}/${session.questions.length} · ${modeLabel(session)} · ✓ ${counts.answered} · ? ${counts.unanswered} · ✕ ${counts.discarded}`));
    add(rule);
    if (!q) add(theme.fg("warning", " No questions loaded. Ask the agent to call dag_grillme_set_questions."));
    else {
    ...
    const buttons = session.mode === "nav"
      ? [rawKeyHint("h", "Prev"), rawKeyHint("l", "Next"), rawKeyHint("j/k", "Option"), rawKeyHint("Enter", "Answer selected"), rawKeyHint("a", "Freeform"), rawKeyHint("c", "Chat"), rawKeyHint("d", "Discard")]
      : session.mode === "answer"
        ? [rawKeyHint("Enter", "Save"), rawKeyHint("Shift+Enter", "Newline"), rawKeyHint("Esc", "Nav")]
        : [rawKeyHint("Enter", "Ask agent"), rawKeyHint("Esc", "Nav")];
    pushWrapped(lines, theme.fg("dim", buttons.map(stripAnsi).join("  ")), width, " ");
    add(rule);
    return [...lines, ...super.render(width)];
  }
}

export function installGrillMeEditor(ctx: ExtensionContext) {
  ctx.ui.setEditorComponent((tui, theme, keybindings) => new GrillMeEditor(tui, theme, keybindings, ctx));
}
