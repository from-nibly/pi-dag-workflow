...
import { assertTurnProjection } from "./projection.mjs";

export const RENDERER_CONTRACT_VERSION = 1;

export function renderTurn(projection) {
...
<body>
  ...
  <header class="topbar">
    ...
    <div class="top-actions"><span class="status-chip">${escapeHtml(projection.statusLabel ?? "Active review")}</span></div>
  </header>
  <div class="layout">
    ...
    <main>
      ${projection.presentationNotice ? `<div class="callout prototype-notice"><strong>About this turn</strong><p>${escapeHtml(projection.presentationNotice)}</p></div>` : ""}
      <section class="hero card" id="understanding">
        <span class="section-label">Current understanding · non-authoritative synthesis</span>
        <div class="understanding-body">${formatParagraphs(projection.currentUnderstanding.body)}</div>
      </section>
...
function renderPoint(projection, point) {
  ...
  if (point.purpose === "awareness") {
    return `<article class="review-card awareness" id="point-${attr(point.id)}">
      ...
      <h3>${escapeHtml(point.title)}</h3><p>${escapeHtml(point.context)}</p>${blocks}
...
function blocksAt(projection, placement) {
...
function renderBlock(block) {
  ...
  if (block.kind === "markdown") return `<div class="presentation-block markdown" data-block-id="${attr(block.id)}">${title}${formatParagraphs(block.body ?? "")}</div>`;
...
function semanticDigest(value) { return `sha256:${createHash("sha256").update(JSON.stringify(value)).digest("hex")}`; }
function formatParagraphs(value) {
  return String(value).split(/\n\s*\n/).map((paragraph) => `<p>${escapeHtml(paragraph).replaceAll("\n", "<br>")}</p>`).join("");
}
function escapeHtml(value) { return String(value ?? "").replace(/[&<>"']/g, (char) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[char]); }
function attr(value) { return escapeHtml(String(value ?? "").replace(/[^A-Za-z0-9._:-]/g, "-")); }
